...
public class BiddingController {
    ...
    public String bidItem(@PathVariable (name="id") int id, Model model) {
        ...
        boolean match= followService.match(userService.findByUsername(userService.getLoggedInUsername()).getId(),item.getId());
        ...
        model.addAttribute("category",item.getCategory().getCategory_name());
...
//        if(userService.findByUsername(username).getCredit_card().getAmount()>=bidForm.getNew_offer()) {
    ...
    public String chargeBuyers()
    {
        ...
        for (ApprovedItem appItem: approvedItems)
        {
            ...
            {
                Item item = itemService.get(appItem.getId());
    ...
    public String deleteSoldItems()
    {
        ...
        for (ApprovedItem appItem: approvedItems)
        {
            if(appItem.getEnd_date().compareTo(curDate)<0)
            {
                approvedItemService.delete(appItem.getId());
                itemService.delete(appItem.getId());
            }
